...
export const addWildcardSectionSchema = object().shape({
  anticipatedSize: number()
    .typeError("global max must be a number")
    .integer()
    .min(0)
  ...
  location: string().nullable(),
  name: string().nullable(),
  number: string().nullable().typeError("number is a required field"),
  prefix: array().of(string().uppercase()).nullable().typeError("prefix is a required field"),
  roomCapacity: number()
    .typeError("room capacity must be a number")
    .positive()
    .integer()
    ...
    .transform(emptyStringToNull)
    .nullable(),
  section: string().nullable().uppercase().typeError("section is a required field"),
  status: string().nullable(),
  studentHours: number()
    .nullable()
    .typeError("student hours must be a number")
